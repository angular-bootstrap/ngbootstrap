    ...
    <nav *ngIf="totalPages >= 1" aria-label="Pagination">
      <ul class="pagination pagination-sm justify-content-center mb-0">
        <li class="page-item" [class.disabled]="page === 1">
          <button class="page-link" type="button" (click)="go(page - 1)" aria-label="Previous">‹</button>
        </li>
        ...
        <li class="page-item" [class.disabled]="page === totalPages">
          <button class="page-link" type="button" (click)="go(page + 1)" aria-label="Next">›</button>
        </li>
      </ul>
    </nav>
  `,
...
export class NgbDgPaginationComponent {
  @Input() page = 1;
  @Input() pageSize = 10;
